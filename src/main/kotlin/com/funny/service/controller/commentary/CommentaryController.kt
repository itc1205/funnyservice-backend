...
class CommentaryController (
    ...
    @GetMapping("/{id}")
    fun getCommentaryById(@PathVariable id: UUID): CommentaryResponse {
        return commentaryService.getById(id).toCommentaryResponse()
    }

    @GetMapping("/fromSlug/{slug}")
    fun getCommentariesFromSlug(@PathVariable slug: String): List<CommentaryResponse> {
        return commentaryService.getCommentariesFromArticle(slug).stream().map { it.toCommentaryResponse() }.toList()
    }
    ...
    fun updateCommentary(@RequestBody commentary: CommentaryUpdateRequest, principal: Principal) {
        ...
        val currentUser = accountService.getByLogin(principal.name)
        if (foundCommentary.creatorId != currentUser.id) {
    ...
    @DeleteMapping("/{id}")
    fun deleteCommentary(@RequestBody id: UUID, principal: Principal) {
        val foundCommentary = commentaryService.getById(id)
        val currentUser = accountService.getByLogin(principal.name)
        if (foundCommentary.creatorId != currentUser.id) {
            throw InsufficientRightsException()
        }
